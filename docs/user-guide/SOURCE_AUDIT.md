...
| Livestream | Guest/live/seller routes; `src/services/live.api.ts`; `back-end/libs/live-commerce/src/presentation/rpc/live-commerce.rpc-controller.ts`; live schema models | Discovery, sessions, comments, vouchers, reminders and provider fields exist | Buyer/seller live articles remain `PARTIAL` until provider/authenticated UAT |
