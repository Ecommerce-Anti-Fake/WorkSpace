...
| AF-DOC-003 | Guest/all roles | Platform selector | Journey Center defaults to the real viewport and permits Desktop/Mobile override | **Passed local and production public controls; mobile-specific override remains locally verified** |
...
| AF-DOC-005 | Seller | Getting-started checklist | Seller Dashboard derives completion from Shop, offer, voucher and order state; request failure does not claim completion | **Passed local state tests/build; authenticated runtime and production visual evidence pending** |
